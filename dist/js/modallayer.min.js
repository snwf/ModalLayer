...
var T=function(){function e(a,b){K(this,e);P(this,"type",null);P(this,"status",null);P(this,"event",null);P(this,"option",null);P(this,"variable",{});try{var c,d,f,g,k,h;null===(c=a.hook)||void 0===c?void 0:null===(d=c.initStart)||void 0===d?void 0:null===(f=d.call)||void 0===f?void 0:f.call(d,a);this.type=a.type;this.status=e._enum.STATUS.HIDE;this.compatibleOption(a);this.initOption(a);this.checkOption();this.linkageOption();this.initVariable();this.initStruct();this.initNode();this.initAttribute();
this.initAnimation();this.initEvent();this.bindEvent();null===(g=this.option.hook)||void 0===g?void 0:null===(k=g.initEnded)||void 0===k?void 0:null===(h=k.call)||void 0===h?void 0:h.call(k,this);this.insertNode()}catch(l){return null===b||void 0===b?void 0:b.call(this,l),this["delete"](),l}}O(e,[{key:"initOption",value:function(a){a.index=e._instance.length;e._instance[a.index]=this;this.option=e._assistant.object.merge(a,e._option.common)}},{key:"compatibleOption",value:function(a){var b,c;e._assistant.object.isString(a.window)&&
(a.window=document.querySelector(a.window));a.position&&(Array.isArray(a.position)||Number.isInteger(a.position)&&(a.position=[a.position,a.position]));if("boolean"===typeof a.mask||["true","false"].includes(a.mask))a.mask={enable:!!a.mask,clickRemove:!0};e._assistant.object.isOnlyObject(a.content)||(a.content={value:a.content,fullContainer:null!==(b=null===(c=a.content)||void 0===c?void 0:c.fullContainer)&&void 0!==b?b:!1})}},{key:"linkageOption",value:function(){0>=this.option.popupTime&&(this.option.progress.enable=
!1);!1===this.option.title&&(this.option.drag.enable=!1)}},{key:"checkOption",value:function(){if(this.option.window&&!(this.option.window instanceof Element))throw new TypeError("option.window does not meet the expected value.");if(Number.isFinite(this.option.transition.animation)){if(!Object.values(e._enum.TRANSITION_ANIMATION_PRESET).includes(this.option.transition.animation))throw Error("No preset animation found.");}else if(null!==this.option.transition.animation&&!(Array.isArray(this.option.transition.animation)||
this.option.transition.animation instanceof Animation))throw Error("Expects a css animation name or Animation object.");if(this.option.position&&!Array.isArray(this.option.position)&&!Object.values(e._enum.POSITION).includes(this.option.position))throw Error("option.position does not meet the expected value.");}},{key:"initVariable",value:function(){this.variable.struct=Object.create(null);this.variable.timeout=Object.create(null);this.variable.interval=Object.create(null);this.variable.animation=
Object.create(null);this.variable.eventSymbol=Object.create(null);this.variable.defaultRect=Object.create(null);this.variable.struct._build=Object.create(null);this.variable.struct._backup=Object.create(null)}},{key:"initStruct",value:function(){var a;if(null===(a=this.option.mask)||void 0===a?0:a.enable)this.variable.struct._build.mask=e._struct.mask;this.variable.struct._build.container=e._struct.container}},{key:"initNode",value:function(){this.variable.nodes=e._assistant.element.objectToNode(this.variable.struct._build);
...
b.setAttribute("modal-layer-type",e._assistant.object.getKeyByValue(e._enum.TYPE,this.type).toLowerCase());b.setAttribute("allow-drag",Number(this.option.drag.enable));b.setAttribute("allow-resize",Number(this.option.resize.enable));if(this.option.progress.enable){a=["top","left","right","bottom"];var g=b.querySelector(".modal-layer-progress-bar");a.includes(this.option.progress.position)||(this.option.progress.position=a[3]);g.setAttribute("progress-bar-position",this.option.progress.position);g.querySelector(".modal-layer-progress-bar-progress").style.cssText+=
"background: "+this.option.progress.color;g.querySelector(".modal-layer-progress-bar-background").style.cssText+="background: "+this.option.progress.background}if(!1!==this.option.title){var k=b.querySelector(".modal-layer-title-content");if(this.option.title instanceof Object){var h=e._assistant.element.objectToNode(this.option.title);Object.keys(h).forEach(function(l){k.appendChild(h[l])})}else k.innerHTML=this.option.title}c&&(c.innerText=this.option.text.interaction.ok);d&&(d.innerText=this.option.text.interaction.no);
f&&(f.innerText=this.option.text.interaction.cancel)}},{key:"initAttribute",value:function(){var a=this.option.ui;var b="modal-layer-skin-"+this.option.skin;var c="modal-layer-index-"+this.option.index;Object.keys(this.variable.nodes).forEach(function(d){for(var f=e._assistant.element.getAllElement(this.variable.nodes[d]),g=0;g<f.length;g++)f[g].classList.contains(a)&&f[g].classList.remove(a),f[g].className="".concat(a," ").concat(f[g].className);this.variable.nodes[d].className=a+" "+b+" "+c+" "+
this.variable.nodes[d].className.trim()+" modal-layer-hide"},this)}},{key:"initAnimation",value:function(){var a=this;var b=Object.create(null);var c=this.variable.nodes;if(null!==this.option.transition.animation){var d=this.option.transition.animation;b.D=[{opacity:0},{opacity:1}];b.container=[[{opacity:0,transform:"scale(.45)"},{opacity:1,transform:"scale(1)"}],[{opacity:0,transform:"translateY(-100%)"},{opacity:1,transform:"translateY(0)"}],[{opacity:0,transform:"rotateY(-120deg)"},{opacity:1,
transform:"rotateY(0)"}],[{opacity:0,transform:"skewX(-100deg)"},{opacity:1,transform:"skewX(0deg)"}],[{opacity:0,transform:"translateY(200%) scale(.45)"},{opacity:1,transform:"translateY(0) scale(1)"}]];var f={fill:"both",id:"modal-layer-transition-animation",easing:this.option.transition.easing,duration:1E3*this.option.transition.duration};this.variable.animation.transition=Object.create(null);Object.keys(c).forEach(function(g){"container"===g?(d instanceof Animation?d.effect.target=c[g]:(d=Array.isArray(d)?
d:b[g][d],d=c[g].animate(d,f)),a.variable.animation.transition[g]=d):a.variable.animation.transition[g]=c[g].animate(b.D,f);a.variable.animation.transition[g].cancel()})}}},{key:"initEvent",value:function(){var a;this.event=e._assistant.object.merge(null!==(a=this.option.event)&&void 0!==a?a:{},e._event)}},{key:"bindEvent",value:function(){var a={once:!1,capture:!1,passive:!1,mozSystemGroup:!1};this.option.mask.enable&&this.option.mask.clickRemove&&(this.variable.eventSymbol.maskClickRemove=e._assistant.event.add(this.variable.nodes.mask,
"click",null,this.event.clickMask,this,null,a));this.option.drag.enable&&(this.variable.eventSymbol.drag=e._assistant.event.add(this.variable.nodes.container.querySelector(".modal-layer-title"),"mousedown",null,this.event.drag,this,null,a));this.option.resize.enable&&(this.variable.eventSymbol.resize=e._assistant.event.add(this.variable.nodes.container,"mousedown",".modal-layer-resize-bar",this.event.resize,this,null,a));this.variable.eventSymbol.active=e._assistant.event.add(this.variable.nodes.container,
...
e._assistant.event.add(b,"click",null,this.event.interaction.ok,this,null,a));this.event.interaction.no&&c&&(this.variable.eventSymbol.interactionNo=e._assistant.event.add(c,"click",null,this.event.interaction.no,this,null,a));d&&(this.variable.eventSymbol.interactionCancel=e._assistant.event.add(d,"click",null,this.event.interaction.cancel,this,null,a))}},{key:"insertNode",value:function(){var a,b=this,c=document.createDocumentFragment(),d=null!==(a=this.option.window)&&void 0!==a?a:window.document.body;
Object.keys(this.variable.nodes).forEach(function(f){return c.appendChild(b.variable.nodes[f])},this);d.appendChild(c)}}]);O(e,[{key:"setStatus",value:function(a){if(Object.values(e._enum.STATUS).includes(a))var b=e._assistant.object.getKeyByValue(e._enum.STATUS,a);else if(b=a.toUpperCase(),void 0===(a=e._enum.STATUS[b]))throw Error("Illegal value");this.status=a;this.variable.nodes.container.setAttribute("modal-layer-status",b.toLowerCase())}},{key:"resize",value:function(){var a=[0,0];var b=this.variable.nodes.container;
if(null===this.option.areaProportion)a=[b.offsetWidth,b.offsetHeight];else{var c=b.children;a[0]=e._assistant.number.multiply(window.innerWidth,this.option.areaProportion[0]);b.style.width=a[0]+"px";for(var d=0;d<c.length;d++)a[1]="absolute"==getComputedStyle(c[d]).position?a[1]:Math.max(e._assistant.element.getNodeHeight(c[d]),a[1]);b.style.height=a[1]+"px"}this.variable.defaultArea=a;this.variable.defaultRect.width=a[0];this.variable.defaultRect.height=a[1]}},{key:"resizeBy",value:function(a,b,
c,d){var f=this.variable.nodes.container;f.style.cssText+="top: ".concat(b,"px; left: ").concat(a,"px; width: ").concat(c,"px; height: ").concat(d,"px;");[e._enum.TYPE.PAGE,e._enum.TYPE.VIDEO,e._enum.TYPE.AUDIO].includes(this.type)&&(f.querySelector("iframe[name="+this.option.layer.name+this.option.index+"]").style.cssText+="width: ".concat(this.option.layer.area[0]+c-this.variable.defaultRect.width,"px; height: ").concat(this.option.layer.area[1]+d-this.variable.defaultRect.height,"px;"))}},{key:"positioning",
value:function(){var a,b;var c=this.variable.nodes.container;var d=this.option.window===document.body?document.documentElement:this.option.window;if(Array.isArray(this.option.position)){var f=J(this.option.position);var g=f[0];f=f[1]}else if(e._enum.POSITION.LEFT_TOP===this.option.position)g=f=0;else if(e._enum.POSITION.RIGHT_TOP===this.option.position)f=0,g=(d?d.offsetWidth:window.innerWidth)-c.offsetWidth;else if(e._enum.POSITION.LEFT_BOTTOM===this.option.position)g=0,f=(d?d.offsetHeight:window.innerHeight)-
c.offsetHeight;else if(e._enum.POSITION.RIGHT_BOTTOM===this.option.position)g=(d?d.offsetWidth:window.innerWidth)-c.offsetWidth,f=(d?d.offsetHeight:window.innerHeight)-c.offsetHeight;else{g=this.variable.defaultRect.width;var k=this.variable.defaultRect.height;f=null!==(a=null===d||void 0===d?void 0:d.scrollTop)&&void 0!==a?a:0;a=null!==(b=null===d||void 0===d?void 0:d.scrollLeft)&&void 0!==b?b:0;b=d?d.clientWidth:window.innerWidth;d=d?d.clientHeight:window.innerHeight;g=e._assistant.number.chain(b).subtract(g).divide(2).add(a).floor().done();
f=e._assistant.number.chain(d).subtract(k).divide(2).add(f).floor().done()}c.style.cssText+="top: ".concat(f,"px; left: ").concat(g,"px;");this.variable.defaultRect.top=f;this.variable.defaultRect.left=g}},{key:"show",value:function(){var a=this;var b=this.variable.nodes;if(!b||0===Object.keys(b).length||this.status===e._enum.STATUS.SHOW)return Promise.resolve();var c=Object.keys(b);var d=this.variable.animation.transition;var f=e._assistant.element.maxZIndex();var g=new Promise(function(k){function h(){0<
a.option.popupTime&&a.event.autoShutdown.call(a);k()}null!==a.option.transition.animation?d.container.onfinish=h:h()});c.forEach(function(k){var h=0>d[k].playbackRate?"reverse":"play";b[k].style.zIndex=f+1;b[k].classList.contains("modal-layer-hide")&&b[k].classList.replace("modal-layer-hide","modal-layer-show");if(null===d||void 0===d?0:d[k])d[k][h]()});this.setStatus("show");return g}},{key:"hide",value:function(){var a=this;var b=this.variable.nodes;if(!b||0===Object.keys(b).length||this.status===
e._enum.STATUS.HIDE)return Promise.resolve();var c=Object.keys(b);var d=this.variable.animation.transition;Number.isInteger(this.variable.timeout.auto_shutdown)&&window.clearTimeout(this.variable.timeout.auto_shutdown);var f=new Promise(function(g){function k(){c.forEach(function(h){b[h].classList.contains("modal-layer-show")&&b[h].classList.replace("modal-layer-show","modal-layer-hide")});a.option.parentModalLayer instanceof e&&a.option.parentModalLayer.show();a.setStatus("hide");g()}null!==a.option.transition.animation?
d.container.onfinish=k:k()});c.forEach(function(g){var k=0<d[g].playbackRate?"reverse":"play";if(null===d||void 0===d?0:d[g])d[g][k]()});return f}},{key:"minimize",value:function(){var a=this;if(this.status!==e._enum.STATUS.MINIMIZE){var b=this.option.transition.duration;var c=document.querySelector("#modal-layer-minimize-queue");c||(c=e._assistant.element.objectToNode([e._struct.minimize_queue])[0],c.classList.add(this.option.ui,"modal-layer-skin-".concat(this.option.skin)),e._assistant.cache.set("minimizeQueueEvent",
...
f);(d=e._assistant.cache.get("minimizeQueueAnimations"))||(d=new Map,e._assistant.cache.set("minimizeQueueAnimations",d));(c=d.get(this.option.index))?c.reverse():(c=[{opacity:0,transform:"scale(.45)"},{opacity:1,transform:"scale(1)"}],b={fill:"both",id:"modal-layer-minimize-queue-transition-animation",easing:this.option.transition.easing,duration:1E3*b},c=f.animate(c,b),d.set(this.option.index,c));this.hide().then(function(){return void a.setStatus("minimize")})}}},{key:"revert",value:function(){var a=
this;if([e._enum.STATUS.MINIMIZE].includes(this.status)){var b=this.option.transition.duration;var c=document.querySelector("#modal-layer-minimize-queue");b=e._assistant.number.divide(b,2);var d=c.querySelector('.modal-layer-minimize-queue-item[modal-layer-index="'+this.option.index+'"]');var f=e._assistant.cache.get("minimizeQueueAnimations").get(this.option.index);setTimeout(function(){a.show()},1E3*b);b=new Promise(function(g){f.onfinish=function(){d.remove();0>=e._minimizeQueue.length&&(c.remove(),
e._assistant.cache.has("minimizeQueueEvent")&&e._assistant.event.remove(e._assistant.cache.get("minimizeQueueEvent")));f.onfinish=null;g()}});f.reverse();return b}}},{key:"remove",value:function(){var a=this;var b=this.status;var c=this.variable.nodes;b===e._enum.STATUS.MINIMIZE&&delete e._minimizeQueue[e._minimizeQueue.indexOf(this)];if(!c||0===Object.keys(c).length||[e._enum.STATUS.HIDE,e._enum.STATUS.REMOVING,e._enum.STATUS.REMOVED].includes(b))return Promise.resolve();this.setStatus(e._enum.STATUS.REMOVING);
return this.hide().then(function(){a.removeAllEvent();Object.keys(c).forEach(function(d){c[d].remove()});a.setStatus("removed")})}},{key:"delete",value:function(){var a=this;this.remove().then(function(){e.a.splice(e.a.indexOf(a),1);e._assistant.object.dereference(a.event);e._assistant.object.dereference(a.option);e._assistant.object.dereference(a.variable);a.event=a.option=a.variable=null})}},{key:"removeAllEvent",value:function(){0!==Object.keys(this.variable.nodes).length&&Object.values(this.variable.eventSymbol).forEach(function(a){return e._assistant.event.remove(a)})}}],
[{key:"removeAll",value:function(){for(var a=0;a<e._instance.length;a++)e._instance[a].remove()}},{key:"msg",value:function(a,b){return e.message(a,b)}},{key:"message",value:function(a,b){"string"===typeof a?a={content:a,type:e._enum.TYPE.MESSAGE}:a.type=e._enum.TYPE.MESSAGE;a=new (e._achieve.get("message"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"tips",value:function(a,b){"string"===typeof a?a={content:a,type:e._enum.TYPE.TIPS}:a.type=e._enum.TYPE.TIPS;
a=new (e._achieve.get("tips"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"alert",value:function(a,b){a.type=e._enum.TYPE.ALERT;a=new (e._achieve.get("alert"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"confirm",value:function(a,b){a.type=e._enum.TYPE.CONFIRM;a=new (e._achieve.get("confirm"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"prompt",value:function(a,b){a.type=e._enum.TYPE.PROMPT;
a=new (e._achieve.get("prompt"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"page",value:function(a,b){a.type=e._enum.TYPE.PAGE;a=new (e._achieve.get("page"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}},{key:"image",value:function(a,b){var c=null;a.type=e._enum.TYPE.IMAGE;c=new (e._achieve.get("image"))(a,b);if(!(c instanceof Error))return c.variable.image.finish.then(function(){c.resize();c.variable.image.layer.resize()}).then(function(){c.positioning();
c.variable.image.layer.positioning()}).then(function(){return c.show()}),c}},{key:"loading",value:function(a,b){a.type=e._enum.TYPE.LOADING;a=new (e._achieve.get("loading"))(a,b);if(!(a instanceof Error))return a.resize(),a.positioning(),a.show(),a}}]);return e}();P(T,"_env",xa);P(T,"_enum",W);P(T,"_struct",V);P(T,"_option",U);P(T,"_variable",new Map);P(T,"_worker",window.Worker?new Map:void 0);P(T,"_event",S);P(T,"_version",1);P(T,"_achieve",new Map);P(T,"_assistant",Object.create(null));
P(T,"_instance",new Proxy([],{set:function(e,a,b){if(b instanceof T||"length"===a)return e[a]=b,!0;throw new TypeError("Must be of type ModalLayer");}}));P(T,"_minimizeQueue",new Proxy([],{set:function(e,a,b){if(b instanceof T){var c=e.indexOf(b);b.minimize();0>c?e[a]=b:e[c]=b;return!0}return"length"===a?(e[a]=b,!0):!1},deleteProperty:function(e,a){a=parseInt(a);if(Number.isInteger(a)&&e[a]instanceof T){e[a].revert();if(e[a+1])for(;a<e.length-1;a++)e[a]=e[a+1];delete e[e.length-1];e.length--;return!0}}}));
Object.seal(T);Object.is(window.ModalLayer,T)?(console.group("ModalLayer already exists"),console.warn("Already version ".concat(window.ModalLayer._version)),console.warn("Try to introduce version: ".concat(T._version)),console.groupEnd()):(Object.preventExtensions(T),Object.defineProperty(window,"ModalLayer",{value:T}));
...
var X=function(){function e(){K(this,e)}O(e,null,[{key:"dereference",value:function(a){a&&Object.keys(a).forEach(function(b){var c=a[b];if(e.isCollection(c))if(e.dereference(c),c instanceof Map)a["delete"](b);else if(c instanceof Set)a["delete"](c);else a[b]=null;else a[b]=null})}},{key:"get",value:function(a,b){b=Array.isArray(b)?b:b.split(".");for(var c=0;c<b.length&&!e.isEmpty(a);c++)a=a[b[c]];return a}},{key:"set",value:function(a,b,c){b=Array.isArray(b)?b:b.split(".");for(var d=0;d<b.length;a=
a[b[d]],d++)e.isEmpty(a[b[d]])&&(a[b[d]]={}),d===b.length-1&&(a instanceof Map?a.set(b[d],c):Object.defineProperty(a,b[d],{value:c,writable:!0,enumerable:!0,configurable:!0}))}},{key:"isEmpty",value:function(a){return null===a||void 0===a}},{key:"isString",value:function(a){return"string"===typeof a||a instanceof String}},{key:"isCollection",value:function(a){var b=[Map,Set,Array,WeakMap,WeakSet,Int8Array,Uint8Array,Int16Array,Int32Array,Uint16Array,Uint32Array,Float32Array,Float64Array,Uint8ClampedArray];
...
T._struct.content_loading_icon;Number.isInteger(Number(this.option.layer.icon))?f.innerHTML[0]=g[this.option.layer.icon]:f.innerHTML[0]["class"]+=" "+this.option.layer.icon;d.innerHTML.push(f);c.innerHTML.push(d)}},{key:"initAttribute",value:function(){E(F(a.prototype),"initAttribute",this).call(this);var c=this.variable.nodes.container;var d=c.querySelector(".modal-layer-content");var f=d.querySelector(".modal-layer-loading-icon");var g=d.querySelector(".modal-layer-loading-box");c.style.cssText+=
...
!1;this.option.mask.enable=!1;this.option.areaProportion=null}},{key:"positioning",value:function(){var c=this.option.layer.iconSize;var d=this.option.layer.position;var f=this.option.layer.location;var g=this.variable.nodes.container.getBoundingClientRect();f=f.getBoundingClientRect();if(d==T._enum.TIPS_POSITION.LEFT){var k=f.left-g.width-c;var h=f.top+(f.height-g.height)/2}d==T._enum.TIPS_POSITION.UP&&(k=f.left-(g.width-f.width)/2,h=f.top-g.height-c);d==T._enum.TIPS_POSITION.DOWN&&(k=f.left-(g.width-
f.width)/2,h=f.top+f.height+c);d==T._enum.TIPS_POSITION.RIGHT&&(k=f.width+f.left+c,h=f.top+(f.height-g.height)/2);this.variable.nodes.container.style.cssText+="top: ".concat(h,"px; left: ").concat(k,"px;")}}]);return a}(Pa);T._achieve.set("tips",Qa);
window.Worker&&T._worker.set("canvasFilter",function(){this.onmessage=function(e){var a,b;e=null===(a=(b=e.data).type)||void 0===a?void 0:a.call(b,e.data);this.postMessage({error:0,buffer:e?e.buffer:void 0},[e.buffer])};this.C=function(){this.postMessage({error:1,message:"Failed to send message."})}});
